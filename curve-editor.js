// ChromaScale — Curve Editor v2
// Canvas-based L/C/H curve editor with constraint boundaries
...
class CurveEditor {
  constructor(container, onChange) {
    ...
    this.channels = {
      L: { color: '#555', label: 'Lightness', min: 0, max: 1, points: [] },
      C: { color: '#D97757', label: 'Chroma', min: 0, max: 0.4, points: [] },
      H: { color: '#6A9BCC', label: 'Hue', min: 0, max: 360, points: [] }
    };
  ...
  draw() {
    ...
    for (const ch of ['H', 'C', 'L']) this._drawCurve(ctx, ch);
    for (const ch of ['H', 'C', 'L']) this._drawPoints(ctx, ch);
  ...
  _drawGrid(ctx) {
    ...
    // X-axis: show step labels at key positions
  ...
  _drawConstraintBands(ctx) {
    ...
    // Draw forbidden zones as red-tinted areas
    // Upper forbidden (above maxL) 
    ctx.fillStyle = 'rgba(220, 38, 38, 0.06)';
    ctx.beginPath();
    
    // Top boundary (maxL line)
    const sorted = [...this.constraintBounds].sort((a, b) => a.t - b.t);
    
    // Draw max boundary as a line
  ...
  _drawHighlightMarkers(ctx) {
    ...
    // Draw a vertical guide line at this t position
    const p = this.padding;
    ...
    // Draw highlighted markers on each curve at this t
  ...
  _onMouseDown(e) {
    ...
    // Clear step highlight on any interaction
    ...
    if (nearest) {
      const pts = this.channels[nearest.channel].points;
      const isLEndpoint = nearest.channel === 'L' && (nearest.index === 0 || nearest.index === pts.length - 1);
      this.dragging = { channel: nearest.channel, index: nearest.index, fixX: isLEndpoint };
